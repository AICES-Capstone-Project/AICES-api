...
//                 // Get all resume applications for scoring
//                 var candidateResumeIds = candidates
//                     .SelectMany(c => c.Resumes?.Where(r => r.IsActive).Select(r => r.ResumeId) ?? Enumerable.Empty<int>())
//                     .Distinct()
//                     .ToList();

//                 var resumeApplications = await _context.ResumeApplications
//                     .Where(ra => ra.JobId == jobId && candidateResumeIds.Contains(ra.ResumeId) && ra.IsActive)
//                     .ToListAsync();
...
//                         if (latestResume == null) return 0m;
//                         var app = resumeApplications.FirstOrDefault(ra => ra.ResumeId == latestResume.ResumeId);
//                         return app?.AdjustedScore ?? app?.TotalScore ?? 0m;
//                     })
//                     .ToList();
...
//                 {
//                     var latestResume = GetLatestResume(c);
//                     var application = latestResume != null 
//                         ? resumeApplications.FirstOrDefault(ra => ra.ResumeId == latestResume.ResumeId)
//                         : null;

//                     ws.Cells[row, 1].Value = rank;
//                     ws.Cells[row, 2].Value = c.FullName;
//                     ws.Cells[row, 3].Value = c.Email;
//                     ws.Cells[row, 4].Value = application?.AdjustedScore ?? application?.TotalScore ?? 0;
...
//                 // Get ResumeApplications for scoring
//                 var resumeIds = allResumes.Select(r => r.ResumeId).ToList();
//                 var allResumeApplications = await _context.ResumeApplications
//                     .Where(ra => ra.JobId == jobId && resumeIds.Contains(ra.ResumeId) && ra.IsActive)
//                     .ToListAsync();
                
//                 var scoredCount = allResumeApplications.Count(ra => ra.TotalScore.HasValue || ra.AdjustedScore.HasValue);
...
//                 var candidateResumeApplications = await _context.ResumeApplications
//                     .Where(ra => ra.JobId == jobId && candidateResumeIds.Contains(ra.ResumeId) && ra.IsActive)
//                     .ToListAsync();
...
//                         var latestResume = GetLatestResume(c);
//                         if (latestResume == null) return 0m;
//                         var app = candidateResumeApplications.FirstOrDefault(ra => ra.ResumeId == latestResume.ResumeId);
//                         return app?.AdjustedScore ?? app?.TotalScore ?? 0m;
//                     })
//                     .ToList();

//                 // Get ResumeApplications for top 5 candidates (for cover page)
//                 var top5ResumeIds = sortedCandidates.Take(5)
//                     .SelectMany(c => c.Resumes?.Where(r => r.IsActive).Select(r => r.ResumeId) ?? Enumerable.Empty<int>())
//                     .Distinct()
//                     .ToList();
//                 var top5ResumeApplications = await _context.ResumeApplications
//                     .Where(ra => ra.JobId == jobId && top5ResumeIds.Contains(ra.ResumeId) && ra.IsActive)
//                     .ToListAsync();
...
//                         page.Content().Element(c => ComposeCoverContent(c, company.Name, job, totalCandidates, parsedCount, scoredCount, shortlistedCount, sortedCandidates.Take(5).ToList(), top5ResumeApplications));
...
//                         var candidateApplication = latestResume != null
//                             ? allCandidateResumeApplications.FirstOrDefault(ra => ra.ResumeId == latestResume.ResumeId)
//                             : null;
...
//                             page.Header().Element(c => ComposeCandidateHeader(c, candidate, currentRank, candidateApplication));

//                             page.Content().Element(c => ComposeCandidateContent(c, candidate, latestResume, candidateApplication));
...
//         private void ComposeCoverContent(IContainer container, string companyName, Job job, int totalCandidates, int parsedCount, int scoredCount, int shortlistedCount, List<Candidate> topCandidates, List<ResumeApplication> top5ResumeApplications)
//         {
...
//                 col.Item().Element(c => ComposeTop5Section(c, topCandidates, top5ResumeApplications));
//             });
//         }
...
//         private void ComposeTop5Section(IContainer container, List<Candidate> topCandidates, List<ResumeApplication> resumeApplications)
//         {
...
//                 // Table for Top 5
//                 col.Item().Element(c => ComposeTop5Table(c, topCandidates, resumeApplications));
//             });
//         }

//         private void ComposeTop5Table(IContainer container, List<Candidate> topCandidates, List<ResumeApplication> resumeApplications)
//         {
...
//         private async Task<ResumeApplication?> GetLatestResumeApplicationAsync(int resumeId, int jobId)
//         {
...
//         private async Task<ResumeApplication?> GetResumeApplicationForCandidateAsync(Candidate candidate, int jobId)
//         {
//             var latestResume = GetLatestResume(candidate);
//             if (latestResume == null) return null;
            
//             return await _context.ResumeApplications
//                 .Where(ra => ra.ResumeId == latestResume.ResumeId && ra.JobId == jobId && ra.IsActive)
//                 .OrderByDescending(ra => ra.CreatedAt)
//                 .FirstOrDefaultAsync();
//         }

//         private void ComposeCandidateHeader(IContainer container, Candidate candidate, int rank, ResumeApplication? application)
//         {
//             var latestResume = GetLatestResume(candidate);
...
//         private void ComposeCandidateContent(IContainer container, Candidate candidate, Resume? latestResume, ResumeApplication? application)
//         {
...
//                 col.Item().PaddingBottom(15).Element(c => ComposeScoreBreakdown(c, latestResume, application));
...
//         private void ComposeScoreBreakdown(IContainer container, Resume? resume, ResumeApplication? application)
//         {
